...
//MODULE_PARM_DESC(key, "Chave definida: ");	
...
static int calc_hash(const unsigned char *data, unsigned int datalen, unsigned char *digest)
{
 ...
 //é utilizada toda vez que o devide é aberto
...
//->>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>/* encrypt data */<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<-//
	...
	for(i = 2 ; i < 18;i++){//permite que os 2 primeiros caracteres da mensagem sejam retirados
		vetaux[i-2]=vetaux[i];
	}
	vetaux[16]='\0';
...
//->>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>//DESCRYPT//<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<-//
    ...
    skcipher = crypto_alloc_skcipher("ecb(aes)", 0, 0);//algoritmo que sera usado
    if (IS_ERR(skcipher)) {
        pr_info("could not allocate skcipher handle\n");
        return PTR_ERR(skcipher);
    }
	...
	for(i = 2 ; i < 18;i++){//2 primeiros caracteres da mensagem são retirados
		vetaux[i-2]=vetaux[i];
	}
	vetaux[16]='\0';
	...
	strcpy(message,buffer2);	
		...
		printk("calculo do hash nervoso: ");
